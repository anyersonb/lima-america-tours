...
<section class="bg-cream-100 px-5 py-10 lg:py-16" aria-labelledby="faq-title">
    <div class="container mx-auto max-w-3xl">
        ...
        <div class="text-center mb-8">
            <p class="text-[10px] uppercase tracking-[0.25em] text-lat-red font-bold">
                {{ app()->getLocale() === 'en' ? 'FREQUENT QUESTIONS' : (app()->getLocale() === 'pt' ? 'PERGUNTAS FREQUENTES' : 'PREGUNTAS FRECUENTES') }}
            </p>
            <h2 id="faq-title" class="mt-2 font-display text-3xl lg:text-4xl text-lat-ink">
            ...
            <div class="flex items-center justify-center gap-2 mt-4 mb-0" aria-hidden="true">
                <span class="h-px w-8 bg-lat-red/50"></span>
                <svg class="w-3 h-3 text-lat-red" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l1.5 8.5L22 12l-8.5 1.5L12 22l-1.5-8.5L2 12l8.5-1.5L12 2z"/></svg>
                <span class="h-px w-8 bg-lat-red/50"></span>
            </div>
        </div>
        ...
        <div class="space-y-2" itemscope itemtype="https://schema.org/FAQPage">
            ...
            @foreach ($faqs as $i => $faq)
                ...
                <div x-data="{ open: {{ $i === 0 ? 'true' : 'false' }} }"
                     class="bg-white rounded-2xl shadow-sm ring-1 ring-lat-ink/5 overflow-hidden"
                     itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">

                    <button type="button"
                            @click="open = !open"
                            :aria-expanded="open"
                            class="w-full flex items-center justify-between gap-4 px-6 py-4 text-left font-semibold text-lat-ink text-sm lg:text-base hover:bg-cream-50 transition"
                            aria-controls="faq-answer-{{ $i }}">
                        <span itemprop="name">{{ $question }}</span>
                        <span class="shrink-0 w-6 h-6 rounded-full bg-lat-red/10 grid place-items-center text-lat-red transition-transform duration-200"
                    ...
                    <div id="faq-answer-{{ $i }}"
                        ...
                        <div class="px-6 pb-5 pt-0 text-sm text-lat-ink-soft/85 leading-relaxed border-t border-cream-100"
                             itemprop="text">
                            {!! nl2br(e($answer)) !!}
                        </div>
                    </div>
