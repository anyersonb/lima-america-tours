...
<div id="{{ $id }}"
     ...
     class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-lat-ink/60 backdrop-blur-sm"
    ...
    <div @click.outside="show = false"
        ...
        <button type="button" @click="show = false"
                class="absolute top-3 right-3 z-10 w-9 h-9 rounded-full bg-white/90 text-lat-ink grid place-items-center hover:bg-lat-surface-2"
                aria-label="Cerrar">
        ...
        <div class="p-8 lg:p-10 flex flex-col">
            <p class="text-[11px] uppercase tracking-[0.25em] text-lat-red font-semibold">{{ $eyebrow }}</p>
            <h2 id="{{ $id }}-title" class="mt-2 font-display text-3xl text-lat-ink leading-tight">{{ $title }}</h2>
            <p class="mt-3 text-sm text-lat-ink-soft/75 leading-relaxed">{{ $description }}</p>
            <form action="{{ route('newsletter.subscribe') }}" method="post" class="mt-5 space-y-3">
                ...
                <input type="email" name="email" required placeholder="Tu correo electrónico"
                       class="w-full rounded-pill border border-lat-line-strong px-5 py-3 text-sm focus:border-lat-red focus:ring-lat-red">
