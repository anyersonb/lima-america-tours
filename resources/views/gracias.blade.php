@php
    ...
    // Logo lockup blanco: esta imagen va sobre foto con overlay oscuro (ver <section> de la derecha).
    $graciasLogoUrl   = $mediaOr('gracias_logo_image', 'assets/logos/logo-america-white.webp');
@endphp
<!DOCTYPE html>
...
<body class="font-sans">
    <main class="min-h-screen grid lg:grid-cols-2">
        <section class="bg-cream-100 flex flex-col justify-center px-8 lg:px-20 py-16">
            <p class="text-[11px] uppercase tracking-[0.25em] text-lat-ink/70 font-semibold">{{ $graciasBadge }}</p>
            <h1 class="mt-4 font-display text-5xl lg:text-7xl text-lat-ink leading-[1.05]">{!! nl2br(e($graciasTitle)) !!}</h1>
            <p class="mt-5 text-sm text-lat-ink-soft/85 leading-relaxed max-w-md">{{ $graciasBody }}</p>
            <a href="{{ url('/' . $locale) }}" class="btn--primary mt-8 self-start">{{ $graciasCta }}</a>
            <div class="mt-12">
                <p class="font-display text-xl text-lat-ink">Follow us</p>
                <ul class="mt-3 flex items-center gap-4 text-lat-red">
                    @foreach (['Instagram','Facebook','TikTok','Vimeo'] as $sn)
                        <li><a href="#" aria-label="{{ $sn }}" class="hover:text-lat-red-deep transition"><svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/></svg></a></li>
                    @endforeach
                </ul>
            </div>
        </section>
        <section class="relative min-h-[40vh] lg:min-h-screen">
            ...
            <div class="relative h-full grid place-items-center text-white">
                <img src="{{ $graciasLogoUrl }}" alt="Lima América Tours" class="h-16 w-auto">
            </div>
        </section>
    </main>
</body>
